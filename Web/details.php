...
// Nạp cấu hình và các hàm cần thiết
...
$product = null;
if (isset($_GET['id'])) {
    $id = intval($_GET['id']); // Chuyển ID về số nguyên để tránh SQL injection
...
<head>
    ...
    <link rel="shortcut icon" href="/Web/admin/product/uploads/avt3.png" type="image/png">
...
<body>
    ...
    <header>
        <a href="/Web/index.php">
            <img src="/Web/images/avt.png" class="logo" alt="Logo" style="width:130px;">
        </a>
        <div id="menu">
            <ul>
                ...
                <li>
                    ...
                    <ul class="sub-menu">
                        ...
                        <li><a href="thucdon_2.php?id_sanpham=3">Lil Wuyn</a></li>
                    </ul>
                </li>
                <li>
                    <a href="#">Khác</a>
                    <ul class="sub-menu">
                        <?php
                        $sql = "SELECT id, name FROM category WHERE id >= 5";
                        $categories = executeResult($sql);
                        if ($categories) {
                            foreach ($categories as $category) {
                                echo '<li><a href="thucdon.php?id_category=' . htmlspecialchars($category['id']) . '">' . htmlspecialchars($category['name']) . '</a></li>';
                            }
                        }
                        ?>
                    </ul>
        ...
        <div class="other">
            <div class="login">
                ...
                if (isset($_SESSION['submit'])) {
                    ...
                    echo '<a style="color:black;">' . htmlspecialchars($user_admin) . '</a>';
                    echo '<div class="logout">';
                    if ($user_admin === 'Admin_Chu') {
                        echo '<a href="/Web/admin/login.php"><i class="fas fa-user-edit"></i> Admin</a><br>';
                    }
            ...
            <li>
                <a href="cart.php" style="text-decoration:none;"><i class="fas fa-shopping-bag"></i></a>
                <?php
                $cart = [];
                if (isset($_COOKIE['cart'])) {
                    $cart = json_decode($_COOKIE['cart'], true);
                }
                $count = array_sum(array_column($cart, 'num'));
                ?>
                <span><?= $count ?></span>
            </li>
        </div>
